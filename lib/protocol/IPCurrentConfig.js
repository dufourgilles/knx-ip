...
module.exports = class IPCurrentConfig {
    ...
    set ip(ip) {
        this._splitIP = KNXUtils_1.splitIP(ip);
    }
    get ip() {
        return this._splitIP.input;
    }
    set mask(mask) {
        this._splitMask = KNXUtils_1.splitIP(mask, 'mask');
    }
    get mask() {
        return this._splitMask.input;
    }
    set gateway(gateway) {
        this._splitGateway = KNXUtils_1.splitIP(gateway, 'gateway');
    }
    set dhcpServer(dhcpServer) {
        this._splitDhcpServer = KNXUtils_1.splitIP(dhcpServer, 'dhcpServer');
    }
    get dhcpServer() {
